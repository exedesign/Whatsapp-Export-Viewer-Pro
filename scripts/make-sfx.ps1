...
if (-not (Test-Path $SourceDir)) { Write-Error "SourceDir yok. Once 'npm run package:portable' calistir"; exit 1 }
if (-not (Get-Command 7z.exe -ErrorAction SilentlyContinue)) { Write-Error "7z.exe PATH'te degil"; exit 1 }
...
Write-Host "Tek dosya hazir: $sfx"
